                                ...
                                if(self.restrictedSpaceViewModel.allRestrictedSpaces.firstIndex(of: restrictedSpace) != self.restrictedSpaceViewModel.allRestrictedSpaces.count-1)
                                   {
                                       if(self.show)//Implemented lazyLoading of list cards,tremendous performance upgrade
                                         {
                                            RestrictedSpaceVerticalCard( taskViewModel:self.taskViewModel,restrictedSpaceId:restrictedSpace.id,taskName1: restrictedSpace.name, difficulty:restrictedSpace.difficulty, startTimeHour:restrictedSpace.startTime.hour,startTimeMinutes:restrictedSpace.startTime.minutes,endTimeHour:restrictedSpace.endTime.hour,endTimeMinutes:restrictedSpace.endTime.minutes, dayOfTheWeek: restrictedSpace.dayOfTheWeek, color: self.restrictedSpaceViewModel.getRestrictedSpaceColor(restrictedSpace: restrictedSpace),offset:self.$offset,id:restrictedSpace.id,isClickable:true).frame(minWidth: 0, maxWidth: .infinity, minHeight: 0, maxHeight: .infinity).padding(EdgeInsets(top: -160, leading: 0, bottom: self.padding, trailing: 0)).offset(y:self.offset).onAppear{self.offset=18}//.onDisappear{self.show=false}
                                       }
                                         else{
