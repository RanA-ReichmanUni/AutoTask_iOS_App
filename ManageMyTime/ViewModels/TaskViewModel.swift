...
class TaskViewModel : ObservableObject
{
    ...
    init(context: NSManagedObjectContext? = nil) {
        // בוחרים context לשימוש
        let usedContext: NSManagedObjectContext
        if let c = context {
            usedContext = c
        } else if let appDelegate = UIApplication.shared.delegate as? AppDelegate {
            usedContext = appDelegate.persistentContainer.viewContext
        } else {
            fatalError("No NSManagedObjectContext available")
        }
        self.managedContext = usedContext
        ...
        // בדיקות עזר שייתנו הודעה ברורה אם המודל לא נטען
        assert(usedContext.persistentStoreCoordinator?.persistentStores.isEmpty == false,
               "Persistent stores not loaded")
        precondition(NSEntityDescription.entity(forEntityName: "CustomDate", in: usedContext) != nil,
                     "Entity 'CustomDate' not found in the Core Data model")

        // ניצור את הישות רק אחרי שה-context תקין
        date = CustomDate(context: usedContext)
        

        firstTaskColor = .white
  ...
  func autoFillTesting() throws
  {
    ...
    func GetDayBoundsSettingsValues () -> [String]
   {
    ...
    func getSettingsValues () -> [Int]
    {
        ...
        switch settingsObject.scheduleAlgorithm {
